					...
					<div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">教学资料信息列表</h4>
                            </div>
                                <div class="card-header">
                                    <form action="MaterialServlet?action=MaterialList"  method="post"  class="d-flex align-items-center">
                                        <div class="mb-2 mx-sm-3">
                                            <input type="text" name="key" value="${key }" class="form-control" placeholder="教学资料主题">
                                        </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-responsive-md" >
                                        <thead> 
                                            <tr>
                                                <th><strong>教学资料主题</strong></th> 
                                                <th><strong>教学资料内容</strong></th> 
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>${data.title }</td>
                                                <td>${data.content }</td>
                                                <td><a style="color:red" href="MaterialServlet?action=downLoad&url=${data.url }">点击下载</a></td>
                                                <td>${data.times }</td>
                                                 <c:if test="${admin!=null }">
                                                <td>
													<div class="d-flex">
														<a href="MaterialServlet?action=toUpdateMaterial&id=${data.id }" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
    ...
    function del(id){
    	if(confirm("确定要删除吗？")){
    		$.ajax({
    			cache:true,
    			type : "post",
    			url : "MaterialServlet?action=deleteMaterial",
    			 data :{"id":id},
    			async:false,
    			success : function(e) {
    				if (e == "yes") {
